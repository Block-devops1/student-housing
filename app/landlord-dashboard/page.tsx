...
export default function LandlordDashboard() {
  ...
  const handleAddProperty = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatusMessage('Error adding property.');
    ...
    <div className="min-h-screen bg-slate-50 py-12">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4">
            ...
            <div className="flex items-center space-x-4">
              <span className="text-gray-700">{landlordData.name || landlordData.email}</span>
              <button
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8 rounded-3xl bg-white p-8 shadow-lg">
          <h2 className="text-3xl font-semibold text-slate-900 mb-2">Manage Your Properties</h2>
          <p className="text-slate-600">
            List new properties, manage applications, and connect with student renters.
          </p>
        </div>
        ...
        <div className="grid gap-10 lg:grid-cols-[1fr_0.9fr]">
          <div className="rounded-3xl bg-white p-8 shadow-lg">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-2xl font-semibold text-slate-900">Your Properties</h3>
              <button
            ...
            {showAddForm && (
              <form onSubmit={handleAddProperty} className="mb-8 rounded-2xl border border-slate-200 p-6 bg-slate-50">
                <div className="space-y-6">
                  ...
                  <label className="block">
                    <span className="text-sm font-medium text-slate-700">Availability date</span>
                    <input
                  ...
                  {statusMessage && (
                    <p className="rounded-2xl bg-green-100 px-4 py-3 text-green-700">{statusMessage}</p>
                  )}
                </div>
              </form>
            )}
            ...
            {properties.length > 0 ? (
              <div className="space-y-4">
                {properties.map((property) => (
                  <div key={property._id} className="rounded-2xl border border-slate-200 p-5 bg-slate-50 hover:border-slate-300">
                    <div className="flex justify-between items-start mb-2">
                      <h4 className="text-lg font-semibold text-slate-900">{property.title}</h4>
                      <span className={`rounded-full px-3 py-1 text-xs font-medium ${property.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700'}`}>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <span className="text-sm text-slate-500 ml-4">{property.bedrooms} bed • {property.bathrooms} bath</span>
                      </div>
          ...
          <aside className="rounded-3xl bg-white p-8 shadow-lg">
            <h3 className="text-2xl font-semibold text-slate-900 mb-6">Student Applications</h3>
            {applications.length > 0 ? (
              <div className="space-y-4">
                {applications.map((app) => {
                  const property = properties.find((p) => p._id === app.propertyId);
                  return (
                    <div key={app._id} className="rounded-2xl border border-slate-200 p-5 bg-slate-50">
                      <p className="text-sm font-medium text-slate-900 mb-2">{property?.title || 'Unknown Property'}</p>
                      <p className="text-sm text-slate-600 mb-3">{app.message || 'Student application'}</p>
                      <div className="flex items-center justify-between">
                        <span
              ...
              <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 py-12 text-center">
                ...
                <p className="text-slate-500 mt-2">Once students apply, they will appear here.</p>
              </div>
            )}
          </aside>
        </div>
