...
const amenitiesOptions = ['generator', 'water supply', 'wifi', 'security', 'laundry', 'parking', 'furnished'];
...
export default function StudentProfilePage() {
  ...
  const [recommendationMessage, setRecommendationMessage] = useState('Enter your preferences and save to see recommended listings.');
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (profile?.preferences?.preferredLocation) setPreferredLocation(profile.preferences.preferredLocation);
      if (typeof profile?.preferences?.maxPrice === 'number') setMaxPrice(String(profile.preferences.maxPrice));
      if (typeof profile?.preferences?.minBedrooms === 'number') setMinBedrooms(String(profile.preferences.minBedrooms));
      if (Array.isArray(profile?.preferences?.preferredAmenities)) setPreferredAmenities(profile.preferences.preferredAmenities);
  ...
  const fetchRecommendations = async () => {
    if (!preferredLocation && !maxPrice && !minBedrooms && preferredAmenities.length === 0) {
      return;
    }
    ...
    if (preferredAmenities.length > 0) params.set('amenities', preferredAmenities.join(','));
    ...
    try {
      ...
      if (data.success) {
        ...
        setRecommendationMessage(data.data.length > 0 ? 'Based on your preferences, these properties match best.' : 'No properties match your saved preferences yet.');
      } else {
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8 rounded-3xl bg-white p-8 shadow-lg">
          ...
          <p className="text-slate-600">
            Save your housing preferences and get recommendations from our Nigerian property listings.
          </p>
        </div>
        ...
        <div className="grid gap-10 lg:grid-cols-[1fr_0.9fr]">
          ...
          <aside className="rounded-3xl bg-white p-8 shadow-lg">
            <h2 className="text-2xl font-semibold text-slate-900 mb-4">Recommended for you</h2>
            <p className="text-slate-600 mb-6">{recommendationMessage}</p>

            {recommendedProperties.length > 0 ? (
              <div className="space-y-4">
                {recommendedProperties.map((property) => (
                  <div key={property._id} className="rounded-3xl border border-slate-200 p-5">
